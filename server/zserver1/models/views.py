from .models import (blister_user, lab_report, blisters,  predefined_precautions,
                     prescription, tablets, tablets_user, lab_report_precautions)
...
class predefined_precautionsList(APIView):
    ...
    def get(self, request, format=None):
        ...
        serializer = predefined_precautionsSerializer(
            predefined_precautionss, many=True)
        return Response(serializer.data)
...
class predefined_precautionsDetail(APIView):
    ...
    def put(self, request, pk, format=None):
        predefined_precautions = self.get_object(pk)
        serializer = predefined_precautionsSerializer(
            predefined_precautions, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class lab_reportList(mixins.ListModelMixin, mixins.CreateModelMixin, generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        print(xml)
        all_data, val1, val2, val3, val4, val5 = lab_reportttt.lab_report_precautions(
            xml)
        # print(data)
        ...
        last_element = lab_report.objects.last()
        last_element.item1 = val1
        last_element.item2 = val2
        last_element.item3 = val3
        last_element.item4 = val4
        last_element.item5 = val5
        last_element.save()
        return created_done
